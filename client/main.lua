...
Citizen.CreateThread(
  function()
    local waitTime = 500
    AddTextEntry("DOPE_PERICO_HELP", "Appuyez sur ~a~ pour prendre un vole vers ~a~")
    CreateNewBlips()
    while true do
      Wait(waitTime)
...
RegisterKeyMapping("+dope_perico", "Téléportation vers/depuis l'île de Cayo Perico", "keyboard", Config.Control.Key)
          ...
          if not IsPlayerTeleportActive() then
            ...
            while IsPlayerTeleportActive() do
              if GetGameTimer() - start > 20000 then
                print("^1Impossible de se téléporter. Signalez-le aux administrateurs du serveur^7")
                if IsScreenFadedOut() then
                  DoScreenFadeIn(0)
                end
                return
...
Citizen.CreateThread(function()
    local hash = GetHashKey('ig_pilot')
    local pedpos = vector3(-1089.71, -2593.91, 19.20)
    ...
    ped = CreatePed('PED_TYPE_CIVMALE', 'ig_pilot', pedpos.x, pedpos.y, pedpos.z, 234.44, false, true)
